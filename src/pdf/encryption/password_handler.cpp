...
bool PasswordBasedEncryptionHandler::can_handle(const PDFEncryptInfo& info) const {
    if (!info.encrypted) {
        return false;
    }
...
bool PasswordBasedEncryptionHandler::check_password(const std::string& password,
    ...
    for (int revision : revisions) {
        ...
        if (standard_security::check_user_password(password, info, revision, key_length_bits)) {
            matched_variant = "Password-Based Encryption (Revision " + std::to_string(revision) + ")";
            return true;
        }
